defmodule ApiWeb.TimeManager.WorkingTimeControllerTest do
  use ApiWeb.ConnCase
  ...
  describe "index" do
    test "lists all working_times", %{conn: conn} do
      conn = get(conn, Routes.time_manager_working_time_path(conn, :index))
  ...
  describe "create working_time" do
    test "renders working_time when data is valid", %{conn: conn} do
      conn = post(conn, Routes.time_manager_working_time_path(conn, :create), working_time: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, Routes.time_manager_working_time_path(conn, :show, id))
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.time_manager_working_time_path(conn, :create), working_time: @invalid_attrs)
  ...
  describe "update working_time" do
    ...
    test "renders working_time when data is valid", %{conn: conn, working_time: %WorkingTime{id: id} = working_time} do
      conn = put(conn, Routes.time_manager_working_time_path(conn, :update, working_time), working_time: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, working_time: working_time} do
      conn = put(conn, Routes.time_manager_working_time_path(conn, :update, working_time), working_time: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete working_time" do
    ...
    test "deletes chosen working_time", %{conn: conn, working_time: working_time} do
      conn = delete(conn, Routes.time_manager_working_time_path(conn, :delete, working_time))
      assert response(conn, 204)

      assert_error_sent 404, fn ->
        get(conn, Routes.time_manager_working_time_path(conn, :show, working_time))
      end
    end
  end
